...
`/workroom/payments` lists unconfirmed payments and paid orders still awaiting board or email delivery. Signed staff sessions can check status and recover; only the owner can release a provider-confirmed failed workroom attempt for a new payment. Failed attempts are archived, not deleted. Unknown attempts cannot be released through this control. Resolve those in the original Square location before collecting payment again. Manual payment recording uses the same order-level attempt key as card/cash, preventing competing app submissions from independently settling an order.
