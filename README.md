...
## The webhook answers Square instead of arguing with it (September 17)

`src/app/api/square/webhook/route.ts` used to require Postgres before it wrote the sale, so a shop running on the memory backend answered 500 to every completed payment and Square redelivered until it gave up. A payload that disagreed with a saved intent threw into the same catch and looped the same way. The signature check now runs first, before the Square connection is resolved and before any storage is touched.

Three outcomes now answer 200. Without a durable payment store, the sale is still recorded and one log line says there is no durable store, because no redelivery will create one. A payload whose reference, location, connection, amount or currency disagrees with the saved intent is written to `devine_payment_attempts.provider_conflict` with the reason, the Square payment id, the amount and the time it was first seen, and it appears on /workroom/payments for the owner. It settles nothing, and it no longer marks the board ticket paid through the register fallback. A payment that settles but whose board row cannot be written is already recorded as money, so recovery finishes it rather than Square.

The mismatch is not written to `devine_payment_reviews`. That table holds one human finding per payment reference, and a machine row in it would occupy a reference the owner may need and make the owner form's replay check treat itself as already answered.

A transient failure still answers 500 on purpose: reading the order lines from Square, reading or writing the payment store, and writing the sale row. The store still dedupes redeliveries by payment id.

## Order notifications are recorded, and recoverable (September 17)

The mail provider is SMTP through nodemailer, a mailbox the shop owns. SMTP has no idempotency key and no way to look a message up afterwards, so the key belongs to this app. `devine_order_notices` holds one row per order and audience with a stable `notice_key`, the attempt count, the dates, the last error and the Message-ID the notice sends under. The row is written before the socket opens, and the same Message-ID rides every attempt for that notice, which is what mail programs thread and deduplicate on. When a send is accepted the provider identifiers are kept too: the queue line the server answered with and the message id it confirmed.

A send whose answer never came back leaves its row marked sending, and the next read turns it into `unconfirmed`. Ordinary recovery skips those, so the automatic path can never put a second copy of a possibly delivered email in the shop inbox. An owner can send one again from /workroom/payments, with the reason written next to the button, and it goes out under the original Message-ID. A notice already delivered is refused, so a second click sends nothing. None of this touches Square, a card or a charge.

/workroom/payments now lists unresolved payment attempts and unresolved notifications together, with the date each started and the date of the last activity on it. Payment attempts still show their own state. Each notice shows what happened in plain words, how many tries it has had, what the mail server said and the error of the last failure. An older paid order with no notice record still reports its two delivery flags as they are.

New storage: table `devine_order_notices`, and the column `devine_payment_attempts.provider_conflict`. Both are created by the same schema step as the attempts table. Back them up with the order board; the notice rows carry order numbers and audience, not card data.

Regression command: `node --test --test-isolation=none tests/*.test.cjs tools/payment-tests/*.test.cjs` (56 tests). The new file is `tools/payment-tests/notification-recovery.test.cjs`, and the shared PGlite fixtures moved to `tools/payment-tests/harness.cjs`. It covers the webhook answering 200 with no store, filing a mismatch, keeping 500 for a storage failure, refusing an unsigned delivery before touching anything, an email that fails after the charge and is retried once without charging again, a send whose record is lost being kept as unconfirmed and repeated only by the owner under the same message id, and three concurrent recoveries delivering each notice once. Fake Square and fake mail throughout.

Still required before this is trusted in the shop: a real SMTP outage and recovery against a controlled inbox, including what the shop mail program does with two copies carrying one Message-ID; a signed Square redelivery of a genuine mismatch in the sandbox; and hosted Postgres concurrency.
