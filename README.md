...
| `src/app/workroom/**` | **The shop's own tool, Phase 2.** The front door is the order board at `/workroom` (web orders land on it by themselves; phone orders get written up on it); the dashboard at `/workroom/dashboard` is the second tab (Day / Week / Month / Year, stat tiles with like-for-like comparisons, one chart of register money pulled live from Square's Payments API when the link is up); the stem tracker at `/workroom/stems`: purchases, shrink with reasons, per-product recipes. `/workroom/week` was the dashboard's first draft and now redirects. Sits outside `/demo` because it is not part of the customer demo and does not move on launch day. |
