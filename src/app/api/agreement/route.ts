...
function recordText(a: AgreementAcceptance): string {
  return [
    ...
    ``,
    `  Build fee ${money(agreement.buildFee)}, paid at the Client's choice: a deposit of`,
    `  ${money(agreement.deposit)} due on acceptance with the balance due on launch, or the full`,
    `  amount on acceptance. Monthly service fee ${money(agreement.monthly)} from the first of the`,
    `  month after launch, or immediately at the Client's option alongside a full build`,
    `  payment. Edit allowance ${agreement.editAllowance}. Additional work`,
    `  ${money(agreement.hourlyRate)}/hour, quoted and approved in advance. Every card`,
    `  payment carries a 3% customer-paid card fee kept by the Client, wherever the`,
    `  card is used (applied by the system on phone and website orders, by the shop`,
    `  at its register); orders placed through the website additionally carry a $0.99`,
    `  platform fee retained by Glazed Web (shown online as one Convenience fee`,
    `  line). Cash sales carry no fee. ${agreement.timeline}`,
    ``,
