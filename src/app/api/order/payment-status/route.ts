...
export async function POST(req: Request) {
  ...
  try {
    ...
    const error=failed?(attempt.result?.status==='NOT_SUBMITTED'?'No payment was submitted. Return to checkout after the shop checks payment setup.':'The payment was declined or canceled. Return to checkout to try another card or payment method.'):pendingMessage;
