...
export default async function PaymentsPage({ searchParams }: { searchParams: Promise<{ notice?: string }> }) {
  ...
  const messages: Record<string, string> = {
    ...
    'notice-partly-sent': 'The email went out, and the mail server refused at least one of the addresses on it. Its line below names them. Nothing was charged.',
    ...
    'storage-unavailable': 'The payment records could not be read, so nothing was sent and nothing changed. Nothing was charged. Try again once the connection is back.',
    'conflict-cleared': 'The Square warning is cleared off this order. No money was moved and the record of it is kept.',
  };
  type AttemptRow = { attempt_key: string; fingerprint: string; state: string; snapshot: PaymentIntent; result: {status?:string;paymentId?:string}|null; fulfilled: boolean; notified: boolean; customer_notified: boolean; created_at: string; updated_at: string; provider_conflict: {reason?:string;paymentId?:string;amountCents?:number|null;seenAt?:number;clearedAt?:number}|null; review_evidence?: string; reviewed_at?: string; waiting_total: string };
  let rows: AttemptRow[] = [];
  ...
  try {
    ...
    // work too, and so is one Square has contradicted and nobody has looked at
    // yet. A conflict the owner has cleared drops off, which is the only way
  ...
  const waiting = Number(rows[0]?.waiting_total ?? rows.length);
  return <section style={{ paddingBlock: 20 }}>
    ...
    {unavailable ? <p role="alert">Payment storage is unavailable. Do not retry charges until the connection is restored.</p> : rows.length === 0 ? <p>No payment recovery work is waiting.</p> : rows.map(row => <section key={row.attempt_key} style={{ borderBottom: '1px solid var(--line)', paddingBlock: 20 }}>
      ...
      {row.provider_conflict && <p role="alert">Square reported a payment on this reference that does not match the saved order, because {conflictWording(row.provider_conflict.reason)}. Square payment {row.provider_conflict.paymentId}{typeof row.provider_conflict.amountCents === 'number' ? ` for $${(row.provider_conflict.amountCents / 100).toFixed(2)}` : ''}, first seen {shopTime(row.provider_conflict.seenAt ?? row.updated_at)} Eastern. Nothing was settled from it. Check this payment at Square before anyone collects again.</p>}
      {/* Somebody has to be able to finish with one of these. Until now nothing
          could clear a filed conflict, so the row sat at the top of a list that
          shows the oldest hundred and pushed newer work off the end of it. */}
      {owner && row.provider_conflict && <form action={clearConflict} style={{ marginTop: 12 }}>
        <input type="hidden" name="key" value={row.attempt_key}/>
        <button className="btn">I checked this payment at Square, clear the warning</button>
      </form>}
      ...
      {owner && noticesOf(notices, row.attempt_key).filter(item => ['failed', 'unconfirmed', 'unconfigured', 'sent-with-refusals'].includes(item.state)).map(item => <form key={`resend-${item.key}`} action={sendNoticeAgain} style={{ marginTop: 12 }}>
        <input type="hidden" name="key" value={row.attempt_key}/>
        ...
        {item.state === 'sent-with-refusals' && <p>One copy of this did go out, to the addresses the mail server took. Sending it again puts a second copy in those inboxes, under the same message id, so most mail programs file the two together. Correct the refused address first or the same thing happens. It cannot charge anyone.</p>}
