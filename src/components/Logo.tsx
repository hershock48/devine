...
/*
  One petal, wrapped FOUR deep on purpose.

  A single element can hold one transform, so a petal animated on one element gets
  one rhythm: it goes where it goes, in a straight line, at one speed. Nesting lets
  independent motions compose, each with its own shape and its own curve:

    drift   translateX, scale, fade   the journey
    float   translateY                the arc, up and over
    tilt    rotateY under perspective the petal turning edge-on into the air
    turn    rotateZ                   one continuous sweep, start to finish

  THE FOURTH LAYER IS NEW AND IT IS THE POINT. Three layers of Z-plane motion still
  read as a sticker being slid around, however well eased, because a flat shape that
  only ever rotates in the picture plane has no thickness. rotateY gives it a face
  and an edge. It is the difference between a decal and something in the air.

  Only transform and opacity are animated, so every frame stays on the compositor.
  An early version animated clip-path, which relayouts on the main thread.

  The stiffness that got this rebuilt was NOT in this file — it was four X keyframes
  in the stylesheet, each re-applying an ease-out, so the petal stopped dead three
  times per flight. The full autopsy is in the "armed" block in globals.css.
*/
function Petal({ variant }: { variant: "lead" | "trail" }) {
  return (
    <span className={`petal petal--${variant}`} aria-hidden="true">
      <span className="petal-float">
        <span className="petal-tilt">
          <svg className="petal-turn" viewBox={PETAL_VIEWBOX} width="2770" height="3100" focusable="false">
      ...
      <span className="brand-plate">
        ...
        */}
        <span className="brand-mask">
          ...
          />
          {/*
            THE CLIP LIVES ON THIS WRAPPER, NOT ON THE VEIL.

            clip-path is resolved in the element's own coordinate space BEFORE its
            transform, so an ellipse on the veil itself travels with the veil — at
            rest it sat over the left of the mark and ate the right half of the oval
            ring, which is the opposite of what it was added to do. Caught in the
            filmstrip's first frame, invisible in the source.

            This wrapper never moves, so its ellipse stays registered to the mark.
            The veil slides inside it and the ring is never covered at any point in
            the animation.
          */}
          <span className="brand-reveal" aria-hidden="true">
            <span className="brand-veil" />
          </span>
        </span>

        {/*
          THE REVEAL IS A VEIL, NOT A CUT. A rectangle in the header's own paper
          colour, with a soft gradient edge, sitting over the wordmark and sliding
          off to the right. The first version cut the image with clip-path, and a
          razor-straight edge travelling across a script wordmark reads as a
          progress bar. A feathered edge in the same colour as the ground behind it
          reads as the letters arriving.
        ...
        */}
        {/*
          ...
          later, and never lines up with the first.

          aria-hidden throughout: the mark's alt text already says the name, and
          announcing these would say it twice.
        */}
        <Petal variant="lead" />
        <Petal variant="trail" />
      </span>
...
/*
  Adds the classes that arm the motion, on mount, once per document load. Two of
  them now: `.is-blowing` on the mark, `.is-drawing` on the hero photograph.

  ONE PREFERENCE CHECK, NOT TWO. Both effects are opt-in from the same guard, so
  there is exactly one place where "does this visitor want motion" is answered and no
    ...
    <script
      ...
      dangerouslySetInnerHTML={{
        __html:
          ...
          "document.querySelectorAll('.breeze').forEach(function(n){n.classList.add('is-blowing')});" +
          "document.querySelectorAll('.draws').forEach(function(n){n.classList.add('is-drawing')})}}catch(e){}",
      }}
    />
  );
}
