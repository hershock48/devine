...
/*
  One petal, wrapped three deep on purpose.

  A single element can hold one transform, so a petal animated on one element gets
  one rhythm: it goes where it goes, in a straight line, at one speed. That is what
  made the first version read as clunky. Nesting lets three independent motions
  compose, each with its own duration and its own easing, and because the periods do
  not divide into each other the result never repeats the same shape twice:

    drift   translateX and fade   the journey
    float   translateY            the rise and the sag, on a longer period
    turn    rotate                a wobble of a few degrees, on a shorter one

  Only transform and opacity are animated, so every frame stays on the compositor.
  The first version animated clip-path, which relayouts on the main thread and is a
  large part of why it stuttered.
*/
