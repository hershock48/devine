 ...
 for(const scenario of ['paid','canceled','missing','board-read','connection-null','connection-change','connection-error','payment-timeout']){
  ...
  try{
   const service=load('src/lib/square/payment-service.ts',{
    './payment-attempts':f.repository,'./payment-engine':engine,'@/lib/intake':{},'./client':{},'./payment-notices':noticeStore,
...
test('owner override through the real service: releases an unknown attempt Square has no record of, refuses one Square reports, refuses completed, refuses staff',async()=>{
 for(const scenario of ['no-record','square-has-payment','completed','staff']) {
  ...
  try {
   ...
   let searches=0;
   const service=load('src/lib/square/payment-service.ts',{
    './payment-attempts':f.repository,'./payment-engine':engine,'@/lib/intake':{},'./payments':{},'./payment-notices':noticeStore,
